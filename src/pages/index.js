...
class Home extends Component {
  ...
  demo(row, cell,) {
    return <input type="radio" name="profile" id={row} onClick={(e) => this.EditProduct(cell)} />
  }
  EditProduct(cell) {
      setTimeout(
            function() {
  ...
  profile(row,cell){
    var st = classes['bg']
  return <div><div className={[classes.pname,st].join(" ")}>{cell.name}</div>{cell.orderinfo}<br/><small>{cell.email}</small></div>
    }
  render() {
    const options = {
          ...
          <div className="row mt-4">
            <div className="col-md-6 col-sm-12">
              ...
              <BootstrapTable data={this.state.tabledata}
                  ...
                  insertRow
                >
                  <TableHeaderColumn dataField='id' hiddenOnInsert={true} isKey={true} width="8%" dataSort={true}  dataFormat={this.demo.bind(this)}></TableHeaderColumn>
                      <TableHeaderColumn dataField='orderinfo' dataSort={true} dataFormat={this.profile.bind(this)}>Product Info</TableHeaderColumn>
                  <TableHeaderColumn dataField='price'  dataSort={true}  >Price Info</TableHeaderColumn>
                  <TableHeaderColumn dataField='description' hiddenOnShow={true} dataSort={true}  >Description</TableHeaderColumn>
                </BootstrapTable>
            </div>
                ...
                this.state.showdata != 'lazy' && this.state.showdata ?
                  <div className="card bg-gray border-0 p-4 bgcolor">
                    <div className="container text-center  p-4">
                      <h4><b>{this.state.showdata.orderinfo}</b></h4>
